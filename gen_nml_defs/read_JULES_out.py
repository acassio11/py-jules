...
def read_JULES_out(base_nml,
    ...
    for prof_nm in l_prof:
        ...
        res_run_id = {**res_run_id, **rncdf.read_ncdf(wd+'/jules_run/namelists/output/'+out_nc_fn, res_CSV)}
    ...
    if time_idx:
        ...
        if res_CSV:
            for k in res_run_id.keys():
                res_run_id[k].to_csv(wd+'/jules_run/namelists/output/'+k+'.csv', index = None, header=True)
    ...
    if clean_res:
        ...
        pft_id      = c.id_crop_pft['n_id'][c.id_crop_pft['crop'] == crop_nm].values[0]
        cpft_id     = c.id_crop_par['n_id'][c.id_crop_par['crop'] == crop_nm].values[0]
